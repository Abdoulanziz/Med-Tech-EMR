...
// Fetch clinical request for dental by visit id
const fetchClinicalRequestForDentalByVisitId = async (req, res) => {
  const visitId = req.params.visitId;
  ...
  try {
    ...
    const result = await ClinicalRequestForDental.findAndCountAll(queryOptions);
...
// Fetch medical history for edntal
const fetchMedicalHistoryForDentalByVisitId = async (req, res) => {
  const visitId = req.params.visitId;
  ...
  try {
    ...
    // Use Promise.all to concurrently fetch data from multiple models
    const [dentalResult] = await Promise.all([
      ClinicalRequestForDental.findAndCountAll(clinicalDentalQuery),
    ]);
    ...
    // Combine results from all models
    const allResults = [ ...clinicalDentalResults];
    ...
    console.error('Error fetching dental medical records:', error);
    return res.status(500).json({ message: 'Internal Server Error' });
  }
};
